...
class WumpusMundo:
    ...
    def _checar_vizinhanca(self, posicao):
        ...
        for adj_pos in adjacentes.values():
            ...
            if adj_pos in self.posicao_buracos:
                percepcoes["brisa suave"] = True
        # Verificar se a posição atual contém o ouro
...
def salvar_registro(jogo, caminho_arquivo="registro.txt"):
    with open(caminho_arquivo, "w") as file:
        file.write(f"Tamanho do mapa: {jogo.tamanho}\n")
        file.write(f"Posição do Wumpus: {jogo.posicao_wumpus}\n")
        file.write(f"Posição do Ouro: {jogo.posicao_ouro}\n")
        file.write(f"Posições dos Buracos: {jogo.posicao_buracos}\n")
